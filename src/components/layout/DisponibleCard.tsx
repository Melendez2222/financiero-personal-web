...
/** Tarjeta compacta "Saldo actual" para el pie del sidebar. */
export function DisponibleCard() {
  ...
  return (
    ...
    >
      <Box sx={{ fontSize: 12, color: colors.textSecondary }}>Saldo actual</Box>
      <Box sx={{ fontSize: 20, fontWeight: 700, color: colors.accent, mt: 0.25 }}>
        {money(disponible)}
      </Box>
      <Box sx={{ fontSize: 11, color: colors.textTertiary, mt: 0.75 }}>
        ingresos − gastos de este mes · {money(usado)} en variables
      </Box>
    </Box>
  );
}
